describe('Testes Pagina de Cursos', () => {
    ...
    it('Filtro de Curso por nome', () => {
        cy.get('input[type=seach').type('Ciências da computação')
        cy.get('table tbody tr').should('have.lenght', 1)
    })
    it('Filtro de Curso por semestre', () => {
        cy.get('input[type=seach').type('8 semestres')
        cy.get('table tbody tr').should('have.lenght', 2)
    })
    it('Ordenação dos cursos por nome', () =>{
        cy.get('th').contains('Cursos').click()
        cy.get('th').contains('Cursos').click()
        cy.get('th').contains('Cursos').click()
        cy.get('table tbody tr:fist-child td:first-child')
